...
export default class KdTree {
  ...
  buildAgentTree() {
    if (this.buildLen != this.simulator.getNumAgents()) {
      this.buildLen = this.simulator.getNumAgents();
      this.agents = [...this.simulator.agents];
      let treeLen = this.agentTree.length;
      let needTreeLen = 2 * this.agents.length;
      if (treeLen > needTreeLen && treeLen > this.MAX_TREE_NODES) {
        this.agentTree.length = needTreeLen;
      } else {
        for (var i = treeLen, len = needTreeLen; i < len; i++) {
  ...
  private _buildAgentTreeRecursive(begin: number, end: number, node: number) {
    ...
    for (var i = begin + 1; i < end; ++i) {
      const px = this.agents[i].position.x
      const py = this.agents[i].position.y

      this.agentTree[node].minX = Math.min(this.agentTree[node].minX, px)
      this.agentTree[node].maxX = Math.max(this.agentTree[node].maxX, px)
      this.agentTree[node].minY = Math.min(this.agentTree[node].minY, py)
      this.agentTree[node].maxY = Math.max(this.agentTree[node].maxY, py)
    }
    ...
    if (end - begin > this.MAXLEAF_SIZE) {
      ...
      if (leftSize == 0 || leftSize == end - begin) {
        ++leftSize;
        ++left;
        ++right;
      }
